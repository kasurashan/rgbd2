...
def build(args):
    ...
    criterion.to(device)
    # postprocessors = {'bbox': PostProcess(processor_dct=processor_dct if (args.with_vector and args.eval) else None)} #########원래 이 방식은 eval할때만 segm 성능 측정함...
    postprocessors = {'bbox': PostProcess(processor_dct=processor_dct if (args.with_vector) else None)} # 그냥 train epoch마다 segm성능 측정하도록 변경

    # if args.masks and args.eval: #########원래 이 방식은 eval할때만 segm 성능 측정함...
    if args.masks:# 그냥 train epoch마다 segm성능 측정하도록 변경
